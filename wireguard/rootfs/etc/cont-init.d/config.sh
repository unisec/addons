...
api="https://consul.unisec.com.br"
...
client_id=$(curl --silent -H "X-Consul-Token: ${token}" ${api}/v1/acl/token/self | jq -r '.AccessorID' | cut -b 29-36)
wireguardjson=$(curl --silent -H "X-Consul-Token:  ${token}" ${api}/v1/kv/${client_id}/smarthome/addons/wireguard.json | jq -r '.[0].Value|@base64d')
...
addresses=$(echo ${wireguardjson} | jq -r '.address')
...
if bashio::config.has_value 'server.private_key'; then
    ...
    server_private_key=$(echo ${wireguardjson} | jq -r '.private_key')
fi
